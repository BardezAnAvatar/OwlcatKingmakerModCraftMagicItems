...
namespace CraftMagicItems.Patches.Harmony
{
    ...
    internal static class WeaponConditionalEnhancementBonusOnEventAboutToTriggerRuleCalculateAttackBonusPatch
    {
        // ReSharper disable once UnusedMember.Local
        private static bool Prefix(WeaponConditionalEnhancementBonus __instance, RuleCalculateAttackBonus evt)
        {
            if (__instance is ItemEnchantmentLogic logic)
            {
                ...
                if (__instance.CheckWielder)
                {
                    ...
                    {
                        if (Validation.EquipmentEnchantmentValid(evt.Weapon, logic.Owner) && __instance.Conditions.Check(null))
                        {
